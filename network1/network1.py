...
A module to implement the mini-batch gradient descent learning algorithm for a feedforward neural network. Gradients are calculated using backpropagation.

The network1.py module includes a class and three functions.

The class is named as ``Network`` which contains methods for computing the gradient by performing the backpropogation and calculating the accuracy by using the trained weights and biases.

The other three functions are the basic functions, one to calculate the sigmoid function, second to calculate the sigmoid prime function(i.e. partial derivative of the sigmoid function) and third to calculate the cost derivative(partial derivative of the cost function).
"""

## Libraries
"""
For our network1.py module we will be using the numpy library for all the numerical computation and random module for making random results.
"""
import numpy as np
import random
...
class Network:
    ...
    def __init__(self, sizes):
        ...
        e.g. if the list is [2, 3, 1] then it would be a three-layer network, with the first layer containing 2 neurons, the second layer 3 neurons, and the third layer 1 neuron.

        The biases and weights for the network are initialized randomly, using a Guassian distribution with mean 0 and variance 1.

        Note that the first layer is the input layer, and by convention we won't set any biases for those neurons, since biases are only ever used in computing the outputs from later layers.
        """
    ...
    def SGD(self, training_data, test_data, epochs, mini_batch_size, eta):
        ...
        The ``test_data``, same as the ``training_data``, is a list of tuples ``(x,y)`` representing the training inputs and the desired outputs. It is there so that we can test the accuracy of our network.

        The parameter ``epochs`` refers to the number of times the learning algorithm will walk through the entire training dataset.
    ...
    def update_mini_batch(self, mini_batch, eta):
        ...
        ## nabla_b is the list that will store the updated value for the biases, i.e. change in the values of biases
        ## every bias's value is initially initialized as 0

        nabla_w = [np.zeros(w.shape) for w in self.weights]
        ## nabla_w is the list that will store the updated values for the weights, i.e. change in the values of weights
